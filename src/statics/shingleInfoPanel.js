...
function shingleInfoPanel(settings) {
	...
	attrNode.className = "shingle-info-attribute";
	this.el.appendChild(attrNode);
	...
	this.setAttr = function (obj, filters) {
	  for(var k in obj) {
      var val = obj[k];
      if(filters[k]) {
        val = filters[k](val)
	...
	this.attrs = {};
	this.renderAttr = function () {
    $(attrNode).children().remove();
    for(var k in this.attrs) {
      var val = this.attrs[k];
      $(attrNode).append($('<span class=\"shingle-info-attr\">' + k + ' : ' + val + '</span>'))
      $(attrNode).append($('<br/>'))
    }
  }
