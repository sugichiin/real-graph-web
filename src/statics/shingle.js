    ...
    graph = function (settings, instance) {
      ...
      var options = settings || {},
        defaults = {
          ...
          explicitcontainerClass : 'shingle-explicit-container',
      ...
      var nodeInfo, makeLineElement,
        ...
        mapinfo = null,
        graphs = {},
        nodeSizeMap = options.nodeSizeMap || {},
      ...
      function setNodeSize(nodeid, size) {
        nodeSizeMap[nodeid] = size;
        var centered = (nodemodeGraph & nodemodeCentered) > 0;
        var nodes = document.querySelectorAll('[data-nodeid="' + nodeid + '"]');
        var range = size == 0 ? 0 : nodeSizeRange(size)
        var nodeRadius = calcNodeRadius(range) * nodeRadiusScale * calcCurrentNodeScale();
        var nEdgeWidth = centered ? 5 * nodeEdgeWidth(range) * nodeEdgeRadiusScale : 0;
        var strokeWidth = 0;
        if(!useCircleEl) {
          strokeWidth = nEdgeWidth > 0 ? (nEdgeWidth + nodeRadius) * 2 : nodeRadius*2;
        } else {
          var nodeScaleFactor = zoomStepsNodeScales[currentScaleStep].factor;
          nodeRadius *= nodeScaleFactor * 3;
          strokeWidth = nEdgeWidth;
        }
        nodes.forEach(function (node) {
          if (node.getAttribute('id').indexOf('highlighted') >= 0) {
            node.setAttributeNS(null, 'data-radius', nodeRadius * 1.5 + 'em');
            node.setAttributeNS(null, 'stroke-width', strokeWidth * 1.5 + 'em');
            if(useCircleEl) {
              node.setAttributeNS(null, 'r', nodeRadius * 1.5 + 'em');
            }
          } else {
            node.setAttributeNS(null, 'data-radius', nodeRadius + 'em');
            node.setAttributeNS(null, 'stroke-width', strokeWidth + 'em');
            if(useCircleEl) {
              node.setAttributeNS(null, 'r', nodeRadius * 1.5 + 'em');
            }
          }
      ...
      function setNodeColor(nodeid, col) {
        nodeColorMap[nodeid] = col;
        var nodes = document.querySelectorAll(['[data-nodeid="' + nodeid + '"]']);
        nodes.forEach(function (node) {
          node.style.stroke = rgbA(col);
        })
      }

      function setEdgeColor(from, to, col, width) {
        edgeColorMap[from] = edgeColorMap[from] || {};
        width = width || '1em';
        edgeColorMap[from][to] = {
          color : col,
          width : width
        };
        var lines = document.querySelectorAll('[data-from="' + from + '"][data-to="' + to + '"]');
        lines.forEach(function (line) {
          line.style.stroke = rgbA(col);
      ...
      var htmlOverlay = (function () {
        ...
        var nodeType = function (settings) {
            ...
            this.nodeEl = null;
            this.radius = 0;
            window.nodes = windows.nodes || {};
            window.nodes[this.nodeid] = window.nodes[this.nodeid] || [];
            window.nodes[this.nodeid].push(this);
            this.rendered = false;
        ...
        function addNode(nodeData) {
          ...
          node = new nodeType({
            ...
            community: nodeCommunity(nodeData)
          });
      ...
      function getMapRect() {
        ...
        return mfrmap.getBoundingClientRect();
      }
      ...
      function nodeColor(node, opacity, explicit) {
        if(explicit != false && node.nodeid != null && nodeColorMap[node.nodeid] != null) {
          return rgbA(nodeColorMap[node.nodeid]);
        }
        return rgbA(nodeColorTuple(node));
      }

      function nodeColorTuple(node) {
        var communityid = nodeCommunity(node),
      ...
      function edgeColor(nodeA, nodeB, unchanged) {
        if (nodeA != null) {
          if(!unchanged && edgeColorMap[nodeA.nodeid] && nodeB != null && edgeColorMap[nodeA.nodeid][nodeB.nodeid] != null ) {
            return rgbA(edgeColorMap[nodeA.nodeid][nodeB.nodeid].color);
          }
          var com = nodeCommunity(nodeA);
          var colorCount = (options.edgeColors || []).length;
          if(!colorCount) {
            return nodeColor(nodeA, 1, false);
          } else {
            return rgbA(options.edgeColors[com % colorCount]);
          }
        } else if (nodeB != null) {
          var com = nodeCommunity(nodeB);
          var colorCount = (options.edgeColors || []).length;
          if(!colorCount) {
            return nodeColor(nodeB, 1, false);
          } else {
            return rgbA(options.edgeColors[com % colorCount]);
          }
        }
      ...
      function edgeHighlightColor(nodeA, nodeB, unchanged) {
        if (nodeA != null) {
          if(!unchanged && edgeColorMap[nodeA.nodeid] && nodeB != null && edgeColorMap[nodeA.nodeid][nodeB.nodeid] != null ) {
            return rgbA(edgeColorMap[nodeA.nodeid][nodeB.nodeid].color);
          }
      ...
      function nodeCommunity(node) {
        if( communityMap[node.nodeid] != null )
          return Number(communityMap[node.nodeid]) || 0;
        return node.community || 0;
      }
      ...
      function edgeWidth(from ,to) {
        if(edgeColorMap[from] != null && edgeColorMap[from][to] != null) {
          return edgeColorMap[from][to].width || '1em';
        }
        return '1em';
      }
      ...
      function quadIntersects(screenrect, root) {
        if(options.drawAll) return true;
      ...
      function findQuadId(nodeid, callback) {
      ...
      function findPosition(nodeid) {
        ...
        findQuadId(nodeid, function (entry) {
          if (entry) {
      ...
      function calcMapStyles() {
        ...
        if (svg12T) {
          ...
          graphCSS.set('.' + options.mapClass + ' .' + options.explicitcontainerClass + ' .' + options.edgeClass, {
            'font-size' : options.edgeWidthFactor + 'px'
          });
        ...
        graphCSS.set('.' + options.mapClass + ' .' + options.explicitcontainerClass, {
          'opacity' : '1'
        });
        ...
        graphCSS.set('.' + options.mapClass + ' svg.with-focus' + ' .' + options.explicitcontainerClass, {
          'opacity' : '0.5'
        });
        ...
        while (stepScale <= maxScale*1.1 && stepScale >= dMinScale) {
      ...
      function renderMap() {
        ...
        renderBase();
        if(options.onInit && !onInitCalled) {
          onInitCalled = true;
          options.onInit();
        }
      }
      function maxZoomStep() {
      ...
      function loadQuad(quadid, callback) {
      ...
      function removeExplicitNode(nodeid) {
        if(explicitNodes[nodeid]) {
          var node = explicitcontainer.querySelector('[data-nodeid="' + nodeid + '"]')
          if(node) node.remove();
          delete explicitNodes[nodeid]
        }
      }
      function removeExplicitEdge(from, to) {
        if(explicitEdges[from] && explicitEdges[from][to]) {
          var edge = explicitcontainer.querySelector('[data-from="' + from + '"][data-to="' + to + '"]')
          if(edge) edge.remove();
          delete explicitEdges[from][to];
        }
      }
      function setExplicitEdge(from, to, length, degree) {
        if(explicitEdges[from] && explicitEdges[from][to]) return;
        function getNodeInfo(nodeid) {
          var elem = explicitcontainer.querySelector('[data-nodeid="' + nodeid + '"]');
          if(!elem) {
            return null;
          }
          return {
            nodeid : String(nodeid),
            quadid : elem.getAttributeNS(null, "data-quadid"),
            x : elem.getAttributeNS(null, "data-x"),
            y : elem.getAttributeNS(null, "data-y"),
            community : elem.getAttributeNS(null, "data-community")
          }
        }
        var fromNode = getNodeInfo(from);
        var toNode = getNodeInfo(to);
        if(fromNode && toNode) {
          if(length && degree == null) {
            var ratio = length / Math.sqrt((toNode.x-fromNode.x)*(toNode.x-fromNode.x) + (toNode.y-fromNode.y)*(toNode.y-fromNode.y));
            var x = Number(fromNode.x) + (toNode.x-fromNode.x)*ratio;
            var y = Number(fromNode.y) + (toNode.y-fromNode.y)*ratio;
            updateNodePos2(toNode, x, y);
            toNode.x = x; toNode.y = y;
          } else if(length && degree != null) {
            var x = Number(fromNode.x) + length * Math.cos(degree);
            var y = Number(fromNode.y) + length * Math.sin(degree);
            updateNodePos2(toNode, x, y);
            toNode.x = x; toNode.y = y;
          }
          var line = makeLineElement(fromNode.x, fromNode.y, toNode.x ,toNode.y, nodeCommunity(fromNode), nodeCommunity(toNode), from, to);
          line.style.stroke = "" + edgeColor(fromNode, toNode);

          line.setAttributeNS(null, "stroke-width", edgeWidth(from, to));
          line.setAttributeNS(null, "data-color", edgeColor(fromNode, toNode, true));

          line.style.zIndex = 1000;
          explicitcontainer.appendChild(line);
          explicitEdges[from] = explicitEdges[from] || {};
          explicitEdges[from][to] = true;
        }
      }
      function unsetNodeColor(node, removeElem) {
        delete nodeColorMap[node];
      }
      function unsetEdgeColor(from, to) {
        if(edgeColorMap[from] && edgeColorMap[from][to]) {
          delete edgeColorMap[from][to];
          var edges = translationEl.querySelectorAll('[data-from="' + from + '"][data-to="' + to + '"]')
          edges.forEach(function (elem) {
            elem.style.stroke = elem.getAttributeNS(null, 'data-color');
            elem.style.strokeWidth = '1em';
          })
        }
      }
      function setExplicitNodes(nodes, callback) {
        var ajaxMap = {};
        var totalCount = 0;
        var out = [];
        nodes.forEach(function (node) {
          var nodeid = Number(node);
          var hash = Math.floor(nodeid / 1000);
          ajaxMap[hash] = ajaxMap[hash] || [];
          ajaxMap[hash].push(nodeid);
          totalCount++;
        })
        if(totalCount == 0) {
          callback && callback([]);
        ...
        var loadCount = 0;
        for(var hash in ajaxMap) {
          (function (hash) {
            ajaxGet(options.graphPath + 'node_' + hash + '.json', function (data) {
              var nodes = JSON.parse(data);
              ajaxMap[hash].forEach(function (nodeid) {
                if(nodes[nodeid]) {
                  var node = nodes[nodeid];
                  out.push(node)
                  if(!explicitNodes[nodeid]) {
                    var circle = MakeNodeElement(node.quadid, node, nodemodeGraph);
                    explicitNodes[nodeid] = true;
                    explicitcontainer.appendChild(circle);
                    circle.style.zIndex = 1000;
                    circle.setAttributeNS(null, "data-community", nodeCommunity(node));
                  }
                }
                loadCount++;
              });
      ...
      function setHighlightNodes(nodes, callback) {
        var ajaxMap = {};
        var totalCount = 0;
        var out = [];
        nodes.forEach(function (node) {
          ...
          (function (hash) {
            ajaxGet(options.graphPath + 'node_' + hash + '.json', function (data) {
              ...
              ajaxMap[hash].forEach(function (nodeid) {
                if(nodes[nodeid]) {
                  var node = nodes[nodeid];
                  var nodeElem = svg.querySelector('[data-nodeid="' + nodeid + '"]')
                  if (nodeElem) {
                    node.x = nodeElem.getAttributeNS(null, 'data-x')
                    node.y = nodeElem.getAttributeNS(null, 'data-y')
                  }
                  out.push(node)
                  if(!highlightednodescontainer.querySelector('[data-nodeid="' + nodeid + '"]')) {
                    var circle = MakeNodeElement(node.quadid, node, nodemodeGraph & nodemodeHighlighted);
      ...
      function highlightTo(nodeid) {
        findQuadId(String(nodeid), function (entry) {
          loadQuad(entry[2], function () {
            changehighlightTo(entry[2], nodeid);
          })
        })
      }
      ...
      function handleMouseMove(evt) {
        ...
        options.onMouseMove && options.onMouseMove(evt, currentScale, svgDims);
      }
      ...
      function addExplicitEvents(container) {
      ...
      function createBaseSvgDOM() {
        ...
        if (options.useBitmap) {
          ...
          loadBitmap("image_2400low.png", "lowRes", function () {
            loadBitmap("image_2400.png", "hiRes");
          });
        ...
        explicitcontainer = document.createElementNS(xmlns, "g");
        explicitcontainer.setAttributeNS(null, "class", options.explicitcontainerClass);

        translationEl.appendChild(explicitcontainer);
        ...
        if (options.selectNodes) {
          ...
          addNodeEvents(explicitcontainer)
        }
      ...
      function showNodeName(quadid, node, elemClass, onHoverOnly, nodeOffSets, mainNode) {
        ...
        if (node.name && node.name != settings.NULLnodeName) {
          ...
          fieldClassName += ' ' + communityClassName(nodeCommunity(node));
      ...
      function makeLineElementEllipticalArc(x1, y1, x2, y2, community1, community2, from, to) {
        ...
        line.setAttributeNS(null,"data-from", from );
        line.setAttributeNS(null, "data-to", to);
        return line;
      }

      function makeLineElementStraight(x1, y1, x2, y2, community1, community2, from, to, exist) {
      ...
      function nodeRange(node) {
        if (!node) return 0;
        if (nodeSizeMap[node.nodeid] != null) return nodeSizeRange(nodeSizeMap[node.nodeid]);
        if (options.ignoreSize) return nodeSizeRange(1);
          ...
          for (j = 0; j < nredges; j++) {
            ...
            if (drawEdge) {
              ...
              var node2 = graphB["nodes"][pos],
                ...
                line = makeLineElement(x1, y1, x2, y2, nodeCommunity(node1), nodeCommunity(node2), node1.nodeid, node2.nodeid);
              ...
              if (drawEdge) {
                ...
                line.setAttributeNS(null, "stroke-width", edgeWidth(node1.nodeid, node2.nodeid));
                line.setAttributeNS(null, "data-color", edgeColor(node1, node2, true))
      ...
      function updateNodePos2(node, x, y) {
        var nodeElems = document.querySelectorAll('[data-nodeid="' + node.nodeid + '"]');
        var lines = document.querySelectorAll(['[data-to="' + node.nodeid + '"],[data-from="' + node.nodeid + '"]']);
        ...
        if(nodeElems && node) {
          ...
          node.y = y
          node.x = x
          nodeElems.forEach(function (nodeEl) {
            nodeEl.setAttributeNS(null, "data-x", node.x);
            nodeEl.setAttributeNS(null, "data-y", node.y);
            if(nodeEl.tagName == "line") {
              nodeEl.setAttributeNS(null, "x1", node.x);
              nodeEl.setAttributeNS(null, "y1", node.y);
              nodeEl.setAttributeNS(null, "x2", node.x);
              nodeEl.setAttributeNS(null, "y2", node.y);
            } else if(nodeEl.tagName == 'circle') {
              nodeEl.setAttributeNS(null, "cx", node.x);
              nodeEl.setAttributeNS(null, "cy", node.y);
            } else if(nodeEl.tagName == 'text') {
              /*
							var tx = Number(nodeEl.getAttributeNS(null, "data-x")) + bx*nodeRadius*0.88;
							var ty = Number(nodeEl.getAttributeNS(null, "data-y")) + by*nodeRadius*0.44;
							nodeEl.setAttributeNS(null, "data-x", tx);
							nodeEl.setAttributeNS(null, "data-y", ty);
							nodeEl.setAttributeNS(null, "x", tx);
							nodeEl.setAttributeNS(null, "y", ty);
							*/
            }
          });
          lines.forEach(function (lineEl) {
            var from = lineEl.getAttributeNS(null, 'data-from');
            var to = lineEl.getAttributeNS(null, 'data-to');
            if(lineEl.tagName == 'line') {
              if(from == node.nodeid) {
                lineEl.setAttributeNS(null, "x1", node.x);
                lineEl.setAttributeNS(null, "y1", node.y);
              } else {
                lineEl.setAttributeNS(null, "x2", node.x);
                lineEl.setAttributeNS(null, "y2", node.y);
              }
            }
            else {
              var d = lineEl.getAttributeNS(null, 'd').split(' ');

              if(from == node.nodeid) {
                var m = d[5].split(',');
                lineEl.setAttributeNS(null, "d", makeD(node.x, node.y, Number(m[0]), Number(m[1])));
              } else {
                var m = d[0].substr(1).split(',');
                lineEl.setAttributeNS(null, "d", makeD(Number(m[0]), Number(m[1]), node.x, node.y));
              }
            }
          })
        }
      ...
      function updateNodePos(quadid, nodeid, x, y) {
        var nodeElems = document.querySelectorAll('[data-nodeid="' + nodeid + '"]');
        var pos = graphs[quadid]["idmap"][nodeid];
        var node = graphs[quadid]["nodes"][pos];
        var lines = document.querySelectorAll(['[data-to="' + nodeid + '"],[data-from="' + nodeid + '"]']);
        var range = nodeRange(node),
          nodeScaleFactor = zoomStepsNodeScales[currentScaleStep].factor,
          // note radius x 3 because node has size radius x 2 and there are always highlighted (x 1,5)
          nodeRadius = calcNodeRadius(range) * nodeRadiusScale * calcCurrentNodeScale() * nodeScaleFactor * 3;
        if(nodeElems && node) {
          ...
          nodeElems.forEach(function (nodeEl) {
            ...
            if(nodeEl.tagName == "line") {
              ...
              /*var tx = Number(nodeEl.getAttributeNS(null, "x")) + bx*nodeRadius;
							var ty = Number(nodeEl.getAttributeNS(null, "y")) + by*nodeRadius;
							nodeEl.setAttributeNS(null, "x", tx);
							nodeEl.setAttributeNS(null, "y", ty);
							nodeEl.setAttributeNS(null, "data-x", tx);
							nodeEl.setAttributeNS(null, "data-y", ty);*/
      ...
      function MakeNodeElementUsingCircle(quadid, node, mode, showNameOnHover) {
        ...
        if (node.name && (node.name != settings.NULLnodeName || !settings.hideNULLnameNodes)) {
          ...
          // base props
          var nodeClass = options.nodeClass + ' ' + communityClassName(nodeCommunity(node));
...
//					circle.setAttributeNS(null, "stroke", "" + nodeEdgeColor(node));
          circle.setAttributeNS(null, "stroke", "rgba(0,0,0,0.4)")
          circle.setAttributeNS(null, "stroke-width", Math.max(10*nodeRadiusScale, nEdgeWid));
      ...
      function MakeNodeElementUsingLine(quadid, node, mode, showNameOnHover) {
        ...
        if (node.name && (node.name != settings.NULLnodeName || !settings.hideNULLnameNodes)) {
          ...
          if(node.nodeid == 107) console.log( color);
        ...
        container.addEventListener('mousedown', function (e) {
          ...
          if (node) {
            ...
            if (name && (name != settings.NULLnodeName || settings.enableNULLnameNodes)) {
              ...
              options.onNodeMouseDown && options.onNodeMouseDown(quadId, nodeId, currentScaleStep, e);
            }
          }
        });
        container.addEventListener('mouseup', function (e) {
          var node = e.target;
          options.onNodeMouseUp && options.onNodeMouseUp(node,e);
          if (node) {
            e.cancelBubble = true;
          }
        });
      ...
      function doScaleTo(value) {
        if(value != currentScaleStep) {
          prevScaleStep = currentScaleStep;
          scaleTo(value, function () {
            findQuadsToDraw();
            findQuadsToRemove();
            zoom.value = value;
          });
      ...
      function createHighlightEdge(quadid, node1, node2) {
        if(highlightedlinescontainer.querySelector('[data-from="' + node1.nodeid + '"][data-to="' + node2.nodeid + '"]')) {
          return false;
        }
        ...
        line.setAttributeNS(null, "stroke-width", edgeWidth(node1.nodeid, node2.nodeid));
        line.setAttributeNS(null, "data-color", edgeHighlightColor(node1, node2, true))
        highlightedlinescontainer.appendChild(line);

        if (highlightednodescontainer != null && node2 != null) {

          // related nodes
          if(!highlightednodescontainer.querySelector('[data-nodeid="' + node2.nodeid + ']')) {
      ...
      function async_showmfrinfo(quadid, nodeid) {
        ...
        this.createEdge = createHighlightEdge;
      ...
      function init() {
        ...
        if (options.zoomSliderElem) {
          zoom = options.zoomSliderElem;
        } else {
      ...
      function drawEdgeForce(from, to) {
      ...
      return {
        ...
        zoomOut: zoomOut,
        zoomReset: zoomReset,
        createHighlightEdge: createHighlightEdge,
        ...
        drawEdge : drawEdgeForce,
        getQuad : getQuad,
        highlightTo : highlightTo,
        graphs : graphs,
        quadsDrawn: quadsDrawn,
        setEdgeColor : setEdgeColor,
        setExplicitNodes : setExplicitNodes,
        setNodeColor : setNodeColor,
        setExplicitEdge : setExplicitEdge,
        removeExplicitEdge : removeExplicitEdge,
        updateNodePos : updateNodePos,
        doScaleTo : doScaleTo,
        unsetNodeColor : unsetNodeColor,
        unsetEdgeColor : unsetEdgeColor,
        setNodeSize : setNodeSize,
        setHighlightNodes : setHighlightNodes,
        maxZoomStep : maxZoomStep,
        removeExplicitNode : removeExplicitNode
      };
